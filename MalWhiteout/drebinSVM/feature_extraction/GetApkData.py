...
def parseargs():
    ...
    parser.add_argument("-cp", "--cpu",
                        ...
                        type=int, required=False, default=psutil.cpu_count())
        ...
        for method in _dex.get_methods():
            ...
            for BasicBlock in g.get_basic_blocks().get():
                ...
                for Instruction in Instructions:
                    URLSearch = re.search("https?://([\da-z\.-]+\.[a-z\.]{2, 6}|[\d.]+)[^'\"]*", Instruction,
                                          re.IGNORECASE)
                    if (URLSearch):
                        URL = URLSearch.group()
                        Domain = re.sub("https?://(.*)", "\g<1>",
                                        re.search("https?://([^/:\\\\]*)", URL, re.IGNORECASE).group(), 0,
                                        re.IGNORECASE)
...
def ProcessingDataForGetApkData(ApkDirectoryPath, ApkFile, output_path, PMap):
    '''
    ...
    try:
        ...
        # Set S6, S5, S7, S8
        CM.ExportToJson(os.path.join(output_path, os.path.splitext(os.path.basename(ApkFile))[0] + ".data"),
                        DataDictionary)
    except Exception as e:
        FinalTime = time.time()
        logger.error(e)
...
def GetApkData(ProcessNumber, output_path, *ApkDirectoryPaths):
    '''
    ...
    for ApkFile in ApkFileList:
        if CM.FileExist(os.path.join(output_path, os.path.splitext(os.path.basename(ApkFile))[0] + ".data")):
            pass
        else:
            ...
            ProcessingResults = pool.apply_async(ProcessingDataForGetApkData,
                                                 args=(ApkDirectoryPath, ApkFile, output_path, PMap),
...
if __name__ == '__main__':
    ...
    output_path = Args.o
    GetApkData(ProcessNumber,output_path, ApkDirectoryPaths)
