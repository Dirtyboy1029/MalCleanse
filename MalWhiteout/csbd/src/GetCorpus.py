...
def GetDataSet(ApkDir, ProcessNo, TimeOut, dst_path='/home/lhd/apk1/'):
    '''
    ...
    OpFNames = [os.path.join(dst_path, os.path.basename(ApkFile.replace('.apk', '.txt'))) for ApkFile in ApkFilesToProcess]
...
if __name__ == "__main__":
    ...
    parser.add_argument('-dst', '-d', type=str, )
    args = parser.parse_args()
    path = args.path
    dst = args.dst
    GetDataSet(path, 24, 12000, dst)
