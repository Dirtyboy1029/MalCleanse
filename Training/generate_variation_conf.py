...
# @Time : 2024/9/10 5:22 
# @Author : DirtyBoy 
# @File : generate_variation_conf.py
import pandas as pd
import numpy as np
import random
...
if __name__ == '__main__':
    ...
    variation = txt_to_list('config/Malradar_variation.mwo')
    thr = pd.read_csv('../Analyze_VT/malware_and_obs.csv')
    variation_thr = []
    for demo1 in variation:
        if int(thr[thr['sha256'] == demo1]['thr'].values[0]) < Threshold:
            variation_thr.append(demo1)
    print(len(variation_thr))
    truth_malware = [item.lower() for item in truth_malware if item.lower() in malware]
    mis_benign = [item.lower() for item in mis_benign if item.lower() in malware]+variation_thr
    ...
    for i in range(1, 4):
        ...
        dump_joblib((data_filenames, gt_labels, noise_labels),
                    'config/databases_thr_variation_' + str(i) + '_' + str(Threshold) + '.conf')
        print(
            'generate conf file to ' + 'config/databases_thr_variation_' + str(i) + '_' + str(Threshold) + '.conf')
