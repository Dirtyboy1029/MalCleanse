...
def customized_reparameterization_dense_layer(scale_factor=1. / 10000):
    ...
    def _mixture_prior_fn_(kernel_size, bias_size=0, dtype=None):
        # Laplace 分布的先验
        laplace_prior = tf.keras.Sequential([
            tfp.layers.VariableLayer(kernel_size + bias_size, dtype=dtype),
            tfp.layers.DistributionLambda(
                lambda loc: tfd.Independent(tfd.Laplace(loc=loc, scale=1),
                                            reinterpreted_batch_ndims=1)),
        ])

        # 正态分布的先验
        normal_prior = tf.keras.Sequential([
            tfp.layers.VariableLayer(kernel_size + bias_size, dtype=dtype),
            tfp.layers.DistributionLambda(
                lambda loc: tfd.Independent(tfd.Normal(loc=loc, scale=1),
                                            reinterpreted_batch_ndims=1)),
        ])

        # 混合分布
        mixture_prior = tfd.MixtureSameFamily(
            mixture_distribution=tfd.Categorical(probs=[0.5, 0.5]),  # 可根据需要调整权重
            components_distribution=[laplace_prior, normal_prior])

        return mixture_prior

    def _laplace_prior_fn(kernel_size, bias_size=0, dtype=None):
        return tf.keras.Sequential([
            tfp.layers.VariableLayer(kernel_size + bias_size, dtype=dtype),
            tfp.layers.DistributionLambda(
                lambda loc: tfd.Independent(tfd.Laplace(loc=loc, scale=1),
    ...
    def _mixture_prior_fn(kernel_size, bias_size=0, dtype=None):
        components = 2  # 混合分布中的分量数量
        return tf.keras.Sequential([
            tfp.layers.VariableLayer(kernel_size + bias_size * components, dtype=dtype),
            tfp.layers.DistributionLambda(
                lambda locs: tfd.MixtureSameFamily(
                    mixture_distribution=tfd.Categorical(logits=tf.zeros(components)),
                    components_distribution=tfd.Independent(tfd.Laplace(loc=locs[..., :components],
                                                                        scale=1),
                                                            reinterpreted_batch_ndims=1))),
        ])
